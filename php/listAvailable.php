...
for ($i=0; $i < count($dagdelen); $i++) {
  $dagdeelTMP = $dagdelen[$i];
...
WHERE
  ...
  ');
  $stmt->bind_param('iiiiii', $dagdeelTMP, $dagdeelTMP, $dagdeelTMP, $dagdeelTMP, $dagdeelTMP, $dagdeelTMP);
  $stmt->execute();
  $stmt->store_result();
  ...
  while ($stmt->fetch()) {
    ...
    $out->klas->$dagdeelTMP[] = $klasObject;
  }
  $stmt->close();
